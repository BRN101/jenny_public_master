			...
			for (songNotes in section.sectionNotes)
			{
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						if(gottaHitNote) 
							sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, boyfriend.noteSkin);
						else
							sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, dad.noteSkin);
